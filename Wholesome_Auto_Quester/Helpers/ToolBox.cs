...
namespace Wholesome_Auto_Quester.Helpers
{
    public static class ToolBox
    {
        ...
        public static bool HostilesAreAround(WoWObject POI, IWAQTask task)
        {
            ...
            if (WholesomeAQSettings.CurrentSetting.BlacklistDangerousZones)
            {
                    ...
                    || hostileUnits.Where(u => u.Key.Level >= me.Level - 2 && poiPosition.DistanceTo(u.Key.Position) < 18).Count() >= maxCount + 1)
                {
                    ...
                    task.PutTaskOnTimeout($"{POI.Name} is surrounded by hostiles", RecoveryPolicy.FirstTimeoutSeconds(TaskFailureKind.SurroundedByHostiles), RecoveryPolicy.Escalates(TaskFailureKind.SurroundedByHostiles));
                    return true;
                }
            }
            return false;
        ...
        public static bool IsObjectiveCompleted(int objectiveId, int questId)
        {
            ...
            {
                // Scripted quests (e.g. the Shaman totem "use item at a spot" steps) can have FEWER quest-log
                // objectives than the index we ask for — an unchecked [objectiveId-1] threw IndexOutOfRange and
                // broke task generation. Treat a missing objective as "not completed".
                return objectiveId - 1 < completionArray.Length && completionArray[objectiveId - 1];
            }
            ...
            {
                return objectiveId - 1 < tempCompArray.Length && tempCompArray[objectiveId - 1];
            }
        ...
        /// <summary>True when a hostile we'd actually fight (see <see cref="GetListObjManagerHostiles"/>) is squarely on
        /// the travel path within <paramref name="maxPathAhead"/> yards ahead. Used to CANCEL an auto-mount: the core
        /// mounts whenever the destination is far and nothing sits in its tight dismount radius, so it would mount up
        /// and then dismount a few yards down the road into a mob already standing on the route (the "mount, ride 6s,
        /// dismount, fight" the bot did). Cheap: point-to-segment geometry against the EXISTING path, no pathfind /
        /// traceline. LoS is ignored on purpose — staying on foot a moment is cheap, and the clearing-path state then
        /// handles the mob properly with full reachability checks.</summary>
        public static bool HostileBlocksTravelPath(float maxPathAhead, float lineWidth)
        {
            List<Vector3> path = MovementManager.CurrentPath;
            if (path == null || path.Count < 2) return false;

            Vector3 myNextNode = MovementManager.CurrentMoveTo;
            int nextIndex = path.IndexOf(myNextNode);
            if (nextIndex < 0) return false;

            // The path from where we ACTUALLY are: me -> next node -> the following nodes, capped at maxPathAhead.
            List<Vector3> forward = new List<Vector3> { ObjectManager.Me.Position, myNextNode };
            float reach = ObjectManager.Me.Position.DistanceTo(myNextNode);
            for (int i = nextIndex; i < path.Count - 1 && reach < maxPathAhead; i++)
            {
                forward.Add(path[i + 1]);
                reach += path[i].DistanceTo(path[i + 1]);
            }

            List<WoWUnit> hostiles = GetListObjManagerHostiles();
            if (hostiles.Count == 0) return false;

            float traveled = 0f;
            for (int i = 0; i < forward.Count - 1 && traveled <= maxPathAhead; i++)
            {
                Vector3 a = forward[i];
                Vector3 b = forward[i + 1];
                foreach (WoWUnit u in hostiles)
                {
                    if (wManagerSetting.IsBlackListedZone(u.Position)) continue;
                    if (WTPathFinder.PointDistanceToLine(a, b, u.Position) <= lineWidth
                        && traveled + a.DistanceTo(u.Position) <= maxPathAhead)
                        return true;
                }
                traveled += a.DistanceTo(b);
            }
            return false;
        }
    }
