...
namespace Wholesome_Auto_Quester.Helpers
{
    class QuestLUAHelper
    {
        ...
        // After interacting with a quest giver the gossip / quest frame opens ASYNCHRONOUSLY (a few 100 ms, and
        // longer right after a turn-in while the previous frame is still closing). Poll briefly for it instead of
        // failing on the very first instant - the old immediate check benched the task for 15 min on a purely
        // transient miss, which lost same-NPC chain follow-ups. Returns true once the frame is active.
        public static bool WaitForQuestGiverFrame(int timeoutMs = 1500)
        {
            ...
            return WTGossip.IsQuestGiverFrameActive;
        }
                ...
                else if (WTGossip.GossipFrameIsVisible)
                {
                    bool isautocomplete = Lua.LuaDoString<bool>($@"
            	        ...
            	        for j=1, GetNumGossipAvailableQuests(), 1 do
            		        local i = j*5-4;
            		        if availableQuests[i] == '{questName.EscapeLuaString()}' or availableQuests[i] == '{questName.EscapeLuaString()} (low level)' then
            	        ...
            	        for j=1, GetNumGossipActiveQuests(), 1 do
            		        local i = j*4-3;
            		        if autoCompleteQuests[i] == '{questName.EscapeLuaString()}' or autoCompleteQuests[i] == '{questName.EscapeLuaString()} (low level)' then
            			        SelectGossipActiveQuest(j);
                                return true;
            		        end
            	        end                         
