...
public class Main : IProduct
{
    ...
    private QuesterOverlay _overlay; // native settings overlay drawn over the game (live edits)
    ...
    public void Start()
    {
        try
        {
            ...
            // Native settings overlay over the game — live edits, its OWN window (decoupled from the AIO3 overlay;
            // both track the WoW window + persist their own position, so they sit side by side without coupling).
            // Hosts the existing ProductSettingsControl (already bound to WholesomeAQSettings, saves on change), so
            // edits apply on the next planner cycle. Guarded inside; if WPF can't start, the normal config window works.
            if (_overlay == null)
            {
                string overlayProfile = null;
                try { overlayProfile = ObjectManager.Me?.Name; } catch { }
                _overlay = new QuesterOverlay("Settings", QuesterOverlaySettings.Build(), overlayProfile);
            }
    ...
    public void Stop()
    {
        try
        {
            ...
            tracker.HideWindow();
            _overlay?.Dispose(); // close the over-the-game settings overlay on stop (re-created on next Start)
            _overlay = null;
            LoggingEvents.OnAddLog -= AddLogHandler;
            EventsLuaWithArgs.OnEventsLuaStringWithArgs -= EventsWithArgsHandler;
            _bot.Dispose();
