...
### 10. Fully scripted Death Knight starting experience
The entire **Death Knight starting chain** — from the first quest at Ebon Hold (map 609), through the relocated Acherus hovering over the Plaguelands, to the faction finale in your capital — is now **AFK-doable end to end**. The base quester can't touch it: almost none of these quests are ordinary kill/gather/talk objectives, so there's nothing to derive from the DB. Instead a **hand-authored ordered "scripted profile"** drives them in strict order — `States/WAQStateDeathKnightStart.cs` + an embedded `Database/DeathKnightStart.json` (**120 steps**), every step verified against the world DB *and* a tested reference run before it ships.

It covers the whole set of one-off DK mechanics as distinct step "actions": the **Eye of Acherus** remote-control vehicle, **runeforging** your first rune, the gargoyle/**frost-wyrm** flights, **"stealing"** channelled loot from Scarlet chests (channel must not be re-interrupted), **persuading**/**duelling** captured Scarlets, **raising ghouls**, the **cannon**, the **Death Gate**, and the climactic **Battle for the Ebon Hold** — including the moment the necropolis **relocates onto the open world (map 0)** — then the faction-specific turn-in at **Thrall / Varian**. It also handles the surrounding runtime hazards the profile can't express as a single objective: pausing WRobot's teleport-detection across the scripted teleports, standing combat/vehicle states down during rides, protecting **every** quest item from the inventory-manager, keeping the drive-by-sell plugin off around the (dead) Acherus vendors, hard-coded training at the DK trainer, and bidirectional **off-mesh** connections up the Lich King ramp and at the finale. Profile-owned quests are managed **entirely** by the executor, in order, so the normal quester never abandons or re-routes them.
...
- **Scripted-profile executor** — new `Bot/ScriptedProfile/` (`ScriptedProfileStep`, `ScriptedProfileData`) + `States/WAQStateDeathKnightStart.cs`, driving the whole **Death Knight start** from an embedded ordered profile (`Database/DeathKnightStart.json`, 120 steps) for the many quests with no DB-derivable objective — vehicles, runeforge, channelled-loot "stealing", persuade/duel, raise-ghouls, cannon, Death Gate, the Ebon Hold battle + open-world necropolis relocation, and the faction finale. Profile quests are owned entirely by the executor so the normal quester never re-routes them.
